...
class ServiceNameFilter(django_filters.FilterSet):
    ...
    def filter_service_name(self, queryset, name, value):
        ...
        relevant_models = [
            ...
            if (not model._meta.abstract and getattr(model, 'service_name_value', '') == value)
        ]
        # ContentTypes use a cached manager, so this should not
        # result in any queries
        content_type_ids = [ctype.id for ctype in ContentType.objects.get_for_models(*relevant_models).values()]
        # Filter the queryset
        return queryset.filter(content_type_id__in=content_type_ids)


class ServiceNameDRFFilter(django_filters.rest_framework.FilterSet, ServiceNameFilter):
    pass
